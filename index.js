...
var himawari = function (userOptions) {
	...
	resolveDate(base_url, options.date, function (err, now) {
		if (err) {
			if (err.code === "ETIMEDOUT") {
				return console.error(
					"Request to Himawari 8 server timed out. Please try again later."
				);
			} else {
		...
		var outfile =
			options.outfile || "./" + [year, month, day, "_", time, ".jpg"].join("");
				...
				async.retry(
					...
					function (inner_cb) {
						...
						stream.on("finish", function () {
							return log("Tile downloaded", uri);
						});
